...
// drawGraph output a dot graph of the entire blockchain generated by the simulation
func drawGraph(height int, ct *chainTracker) {
...
func analyzeSim(ct *chainTracker) {
    fmt.Printf("Sim produced %d blocks\n", len(ct.blocks))
    fmt.Printf("Height 2 has %d blocks\n", len(ct.blocksByHeight[2]))
    fmt.Printf("Height 11 has %d blocks\n", len(ct.blocksByHeight[11]))
}
...
func main() {
        ...
        analyzeSim(chainTracker)
        drawGraph(currentHeight, chainTracker)
}
