...
func drawChain(ct *chainTracker, name string) {
	fmt.Printf("Writing Graph\n")
	fil, err := os.Create(fmt.Sprintf("%s.dot", name))
	if err != nil {
		panic(err)
	}
	defer fil.Close()
...
func runSim(totalMiners int, roundNum int, lbp int, c chan *chainTracker) {
	...
	for round := 0; round < roundNum; round++ {
		...
		// TODO Zenground0 does not understand this and thinks this causes excessive computation
		// because next round the miner tries to mine again on these blocks and sees nothing.
		// Also maybe this causes duplicates / the occasional out of sync heights in the block mining process?
		if len(newBlocks) > 0 {
			printSingle(fmt.Sprintf("\n"))			
			blocks = newBlocks
		} else {
			printSingle(fmt.Sprintf("No new blocks in a round \n"))			
		}
	}
	// height is 0 indexed
...
func main() {
	...
	fName := flag.String("id", "output", "prefix on output files") 
	...
	for result := range c {
		...
		drawChain(result, fmt.Sprintf("%s-rounds=%d-lbp=%d-trial=%d", name, roundNum, lbp, len(cts)))
	}
}
